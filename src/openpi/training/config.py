...
class DataConfig:
    ...
    # Video backend to use for decoding (e.g. "pyav", "torchcodec").
...
class LeRobotUR5DataConfig(DataConfigFactory):
    """UR5 single-arm dataset in LeRobot format (3 cameras + 7D state/action)."""
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False),
        data=LeRobotLiberoDataConfig(
            repo_id="/inspire/hdd/project/exploration-topic/public/zzhuai/data/libero",
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=False,
        ),
        batch_size=256,
    ...
    TrainConfig(
        name="pi05_ur5e_pick_cola_local",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False),
