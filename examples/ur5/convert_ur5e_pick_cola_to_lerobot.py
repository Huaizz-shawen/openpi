"""
Convert UR5e pick-cola raw sessions to LeRobot format for OpenPI fine-tuning.

Expected raw layout:
  <raw_root>/
    session_xxx/
      data/dataset_xxx.json
      videos/
        front_camera_*.mp4
        left_camera_*.mp4
        right_camera_*.mp4
...
This script:
- uses arm_left only
- builds observation.state from arm_left joint positions (7D, including gripper joint)
- builds action as next-step absolute joint position (7D)
- writes a train dataset and (optionally) a validation dataset with session-level split

Example:
  uv run python examples/ur5/convert_ur5e_pick_cola_to_lerobot.py \
    --raw-root /Users/kazusa/Documents/openpi/ur5e_pick_cola \
    --repo-id-train local/ur5e_pick_cola_train \
    --repo-id-val local/ur5e_pick_cola_val \
    --task "pick up the cola and place into basket." \
    --val-ratio 0.1
"""
...
def discover_sessions(raw_root: Path) -> list[SessionData]:
    ...
    for session_dir in sorted(raw_root.glob("session_*")):
        ...
        try:
            ...
            right = _find_video(videos_dir, "right")
...
def split_sessions(sessions: list[SessionData], val_ratio: float, seed: int) -> tuple[list[SessionData], list[SessionData]]:
    ...
    n_val = max(1, int(round(len(sessions) * val_ratio)))
    n_val = min(n_val, len(sessions) - 1) if len(sessions) > 1 else 0
    val = sessions[:n_val]
...
def _build_episode_arrays(session: SessionData, image_width: int, image_height: int, normalize_gripper: bool) -> dict[str, list[np.ndarray]]:
    entries = _load_json_entries(session.json_path)
    states = [_extract_state(e) for e in entries]

    # Absolute next-step action; final action repeats final state.
    actions = states[1:] + [states[-1]]

    if normalize_gripper:
        # Global observed range from this dataset family.
    ...
    cap_right = cv2.VideoCapture(str(session.right_video))
    if not (cap_front.isOpened() and cap_left.isOpened() and cap_right.isOpened()):
        raise RuntimeError(f"Failed to open videos for {session.session_id}")

    try:
        front_iter = _iter_video_frames(cap_front)
        left_iter = _iter_video_frames(cap_left)
        right_iter = _iter_video_frames(cap_right)
        ...
        for _ in range(len(states)):
            ...
            if right is None:
                if last_right is None:
                    raise RuntimeError(f"right video has no frames: {session.right_video}")
                right = last_right

            last_front, last_left, last_right = front, left, right

            out_front.append(_resize_rgb(front, image_width, image_height))
            out_left.append(_resize_rgb(left, image_width, image_height))
            out_right.append(_resize_rgb(right, image_width, image_height))
            ...
            ds.add_frame(
                {
                    ...
                    "observation.images.cam_right": arrays["right"][i],
                    "task": task,
                }
            )
        ds.save_episode()
...
def main():
    parser = argparse.ArgumentParser(description="Convert UR5e pick-cola raw sessions to LeRobot format.")
    parser.add_argument("--raw-root", type=Path, required=True)
    parser.add_argument("--repo-id-train", type=str, required=True)
    parser.add_argument("--repo-id-val", type=str, default="")
    parser.add_argument("--task", type=str, default="pick up the cola and place into basket.")
    ...
    parser.add_argument("--camera-dtype", choices=["image", "video"], default="image")
    ...
    if not sessions:
        raise RuntimeError(f"No valid sessions found under {args.raw_root}")
    ...
    if val_sessions:
        print("Val IDs:", ", ".join(s.session_id for s in val_sessions))
    ...
    if val_sessions and args.repo_id_val:
        _write_dataset(
            sessions=val_sessions,
            repo_id=args.repo_id_val,
            task=args.task,
            fps=args.fps,
            image_w=args.image_width,
            image_h=args.image_height,
